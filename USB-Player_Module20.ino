...
MP3-Player using DFPlayer mini, ATtiny85, USB, LDR or push button switch, 
...
ing features.
 ...
 - Module status (Error, Ready, Play Mode, Volume) via onboard LED.
 - Powered by standard Micro USB 1.0 A phone charger.
 - Uses up to 32GB Micro SD to save MP3 songs/audio clips.
 - Emulates external SD drive when connected to computer using USB cable,
   for easy file transfer.
 - Arduino programming of ATTINY available using ISCP port.

Module commands are entered by waving hand over the LDR sensor, or press-
ing push button switch. Only one switching option should be installed.
A command is considered complete if no addditional input is received
within 1/2 seconds.
  
Selecting the Play Mode or Player Volume commands will cycle to the
next option.
  
Commands:

  1st button press + release = Play next file, randomly selected.
  1st button hold (0.5 s)    = Stop playing current song.
  2nd button press + release = Change play mode (25 sec, full song).
  3rd button press + release = Change player volume (Low, Med, High).
  ...
  1 blink  = Stop playing after 25 sec, or player volume is Low.
  2 blinks = Play the entire song, or player volume is Medium.
  3 blinks = Player volume is High.
...
Update USB Micro SD Player Module menu, to reduce number of button presses,
and add new mode and options.

Key . = Short Press, - = Long Press, * = default
  
 .    = Play song, or stop current song and play new random song.
 -    = Stop playing song, and fade to silence.

 ..   = Change play mode (entire song*, 20 sec only).
 . -  = Change volume (low, med*, high)

 ...  = Enable autoplay mode (off*, on)
 ..-  = Change interval (10sec*, 20sec, 30sec, 60sec, random)

Revision 19:

Clean up timing issues, and songs repeating in autoplay mode.


Revision 20:

Simplify menu structure from 7 commands down to 5.

 .    = Play next song.
 -    = Stop playing song, and fade to silence.

 ..   = Play previous song.
 .-   = Change play mode (entire song, 20 sec, auto, book).
